...
import { bccDbSyncService, bccNodeService, DockerClient, postgresService, StatefulService } from './'
...
export class DockerComposeStack {
  ...
  constructor (
    ...
    this.projectName = options.projectName || 'bcc-graphql'
    this.store = store
    this.statefulServices = options.statefulServices || new Map([
      ['bcc-db-sync', bccDbSyncService(this.projectName)],
      ['bcc-node', bccNodeService(this.projectName)],
