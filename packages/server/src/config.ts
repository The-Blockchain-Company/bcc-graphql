import { Config as ApiBccDbHasuraConfig } from '@bcc-graphql/api-bcc-db-hasura'
import { MissingConfig } from './errors'
import fs from 'fs-extra'
import { Config as ServerConfig } from './Server'
import { LogLevelString } from 'bunyan'

export type Config = ServerConfig & ApiBccDbHasuraConfig & {
...
export async function getConfig (): Promise<Config> {
  ...
  if (!env.bccNodeConfigPath) {
    throw new MissingConfig('BCC_NODE_CONFIG_PATH env not set')
  }
  ...
  return {
    ...
    pollingInterval: {
      bccSupply: env.pollingInterval.bccSupply || 1000 * 60
    },
...
function filterAndTypecastEnvs (env: any) {
  ...
  return {
    ...
    bccNodeConfigPath: BCC_NODE_CONFIG_PATH,
    ...
    pollingInterval: {
      bccSupply: Number(POLLING_INTERVAL_BCC_SUPPLY)
    },
    postgres: {
      db: POSTGRES_DB,
