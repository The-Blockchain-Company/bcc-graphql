...
import { RunnableModuleState } from '@bcc-graphql/util'
...
export class Server {
  ...
  async start () {
    ...
    this.syncProgress = setIntervalAsync(async () => {
      ...
      if (result.data.bccDbMeta.initialized) {
        this.logger.info({ module: 'Server' }, 'DB ready')
        // Promise not awaited purposely
        // https://github.com/The-Blockchain-Company/bcc-graphql/issues/459
        clearIntervalAsync(this.syncProgress)
      } else {
        this.logger.info({ module: 'Server' }, `DB sync progress: ${result.data.bccDbMeta.syncPercentage} %`)
      }
    }, 5000)
  }
