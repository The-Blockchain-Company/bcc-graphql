...
describe('transactions', () => {
  ...
  it('Returns transactions by hashes - aurum -qa', async () => {
    const zerepochResult = await aurum QaClient.query({
      query: await loadQueryNode('transactionsByHashesOrderByFee'),
      variables: {
        hashes: [
    ...
    const timelockResult = await aurum QaClient.query({
      query: await loadQueryNode('transactionsByHashesOrderByFee'),
      variables: {
        hashes: [
    ...
    expect({ zerepochResult, timelockResult }).toMatchSnapshot()
  })
