...
import { BccNodeClient } from './BccNodeClient'
  ...
  return makeExecutableSchema({
    resolvers: Object.assign({}, scalarResolvers, customFieldsComplexity, {
      Mutation: {
        submitTransaction: {
          resolve: async (_root, args) => {
            try {
              const hash = await bccNodeClient.submitTransaction(
                args.transaction
              )
              return { hash }
            } catch (error) {
      ...
      Query: {
        ...
        bcc: {
          resolve: async () => {
            const bccPots = hasuraClient.bccPotsToCalculateSupplyFetcher.value
            if (bccPots === undefined || bccPots === null) {
              return new ApolloError(
                'bcc query results are not ready yet. This can occur during startup.'
              )
            }
            return {
              supply: {
          ...
          selectionSet: null,
          extensions: getComplexityExtension('Query', 'bcc')
        },
        assets: {
          resolve: (_root, args, context, info) => {
            return delegateToSchema({
        ...
        bccDbMeta: {
          resolve: async () => {
            try {
              const slotNo = await bccNodeClient.getTipSlotNo()
              return hasuraClient.getMeta(slotNo)
            } catch (error) {
              throw new ApolloError(
                error.name === 'ModuleIsNotInitialized'
                  ? 'bccDbMeta query is not ready. Try again shortly'
                  : error.message
              )
            }
          },
          selectionSet: null,
          extensions: getComplexityExtension('Query', 'bccDbMeta')
        },
        collateralInputs: {
          resolve: (_root, args, context, info) => {
            return delegateToSchema({
