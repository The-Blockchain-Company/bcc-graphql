...
import util, { DataFetcher, ModuleState } from '@bcc-graphql/util'
...
export type BccPotsToCalculateSupply = { circulating: AssetSupply['circulating'], reserves: BccPots['reserves']}
...
export class HasuraClient {
  ...
  public bccPotsToCalculateSupplyFetcher: DataFetcher<BccPotsToCalculateSupply>
  ...
  constructor (
    ...
    this.bccPotsToCalculateSupplyFetcher = new DataFetcher<BccPotsToCalculateSupply>(
      'BccPotsToCalculateSupply',
      () => {
        try {
          return this.getBccPotsToCalculateSupply()
        } catch (error) {
    ...
    this.client = new GraphQLClient(
      ...
      {
        headers: {
          'X-Hasura-Role': 'bcc-graphql'
        }
      }
    )
  }

  private async getBccPotsToCalculateSupply (): Promise<BccPotsToCalculateSupply> {
    ...
    if (epochs.length === 0 || epochs[0]?.bccPots === null) {
    ...
    return {
      ...
      reserves: epochs[0]?.bccPots.reserves
    }
  }
  ...
  public async initialize () {
    ...
    await this.bccPotsToCalculateSupplyFetcher.initialize()
    this.state = 'initialized'
    this.logger.info({ module: 'HasuraClient' }, 'Initialized')
  }

  public async shutdown () {
    await this.bccPotsToCalculateSupplyFetcher.shutdown()
  }
  ...
  public async getCurrentProtocolVersion (): Promise<SophieProtocolParams['protocolVersion']> {
    const result = await this.client.request(
      gql`query {
          epochs (limit: 1, order_by: { number: desc }) {
              protocolParams {
  ...
  public async getMostRecentPointWithNewAsset (): Promise<Schema.Point | null> {
    ...
    // and bcc-db-sync's which managed the block table.
    await pRetry(async () => {
  ...
  public async getMeta (nodeTipSlotNumber: number) {
    ...
    const { tip } = result?.bcc[0]
    const lastEpoch = result?.epochs[0]
    const syncPercentage = tip.slotNo / nodeTipSlotNumber * 100
    return {
      // bcc-db-sync writes the epoch record at the end of each epoch during times of bulk sync
  ...
  public async getAssetMetbcctaHashesById (assetIds: Asset['assetId'][]): Promise<AssetMetadataHashAndId[]> {
    const result = await this.client.request(
