...
import { ModuleState } from '@bcc-graphql/util'
...
export class Db {
  ...
  public async init ({
    ...
    this.pgSubscriber.notifications.on('bcc_db_sync_startup', async payload => {
      switch (payload) {
        case 'init' :
          this.logger.warn({ module: 'Db' }, 'pgSubscriber: bcc-db-sync-extended starting, schema will be reset')
          await onDbInit()
          break
        case 'db-setup' :
          await onDbSetup()
    ...
    try {
      ...
      await this.pgSubscriber.listenTo('bcc_db_sync_startup')
