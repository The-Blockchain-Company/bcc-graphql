...
import util, { DataFetcher, errors, ModuleState } from '@bcc-graphql/util'
import {
  ConnectionConfig,
...
const MODULE_NAME = 'BccNodeClient'

export class BccNodeClient {
  readonly networkParams: string[]
  public bccCirculatingSupply: AssetSupply['circulating']
  ...
  // public async getProtocolParams (): Promise<Schema.ProtocolParametersSophie> {
  //   if (this.state !== 'initialized') {
  ...
  public async initialize (ogmiosConnectionConfig?: ConnectionConfig) {
    ...
    await pRetry(async () => {
      ...
      onFailedAttempt: util.onFailedAttemptFor(
        'Establishing connection to bcc-node',
        this.logger
      )
    })
