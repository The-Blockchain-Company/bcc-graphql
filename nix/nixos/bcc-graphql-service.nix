...
{ lib, pkgs, config, ... }:
let
  cfg = config.services.bcc-graphql;
  selfPkgs = import ../pkgs.nix {};
in {
  options = {
    services.bcc-graphql = {
      enable = lib.mkEnableOption "bcc-explorer graphql service";
      ...
      bccNodeConfigPath = lib.mkOption {
      ...
      pollingIntervalBccSupply = lib.mkOption {
        type = lib.types.nullOr lib.types.int;
        default = null;
      };
  ...
  config = let
    ...
    frontend = selfPkgs.packages.bcc-graphql;
  ...
  in lib.mkIf cfg.enable {
    systemd.services.bcc-graphql = {
      ...
      environment = lib.filterAttrs (k: v: v != null) {
        BCC_NODE_CONFIG_PATH = cfg.bccNodeConfigPath;
      ...
      (lib.optionalAttrs (cfg.pollingIntervalBccSupply != null) { POLLING_INTERVAL_ADA_SUPPLY = toString cfg.pollingIntervalBccSupply; }) //
